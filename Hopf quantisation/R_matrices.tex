\section{The R-matrix formalism}
    \subsection{Reconstructing bialgebras from their representation categories}
        ...
        \begin{definition}[Fibre functors] \label{def: fibre_functors}
            A \textbf{fibre functor} on $(\C, \tensor, \1)$ is an \textit{exact} tensor functor:
                $$F: \C \to k\mod^{\fr}$$
            (wherein $k\mod^{\fr}$ is a tensor category over $k$ in the usual manner). The associative $k$-algebra $\End_{\Mon\Nat}(F)$ is called the \textbf{Tannaka algebra} of $\C$.
        \end{definition}
    ...
    \subsection{Braided monoidal categories and quasi-cocommutative bialgebras}

    \subsection{R-matrices and bialgebra deformations}
        Now, we move on to studying R-matrices, monoidal braidings that can be classified using Hochschild cohomology of bialgebras. Suppose that $H$ is a bialgebra therein. At first, $R$-matrices on $H$-modules, say $V$, shall appear as certain automorphisms of $V^{\tensor 2}$ satsifying equations following a certain \say{Yang-Baxter} formula, originating from scattering theory in statistical mechanics. However, Drinfeld's key insight was that said Yang-Baxter equations are precisely the equations describing bialgebra $2$-cocycles:
            $$R \in \HH^2(H, H^{\tensor 2})$$
        ...
        \begin{convention}
            A convention that will be observed throughout is the following. Suppose that $\C$ is a monoidal category and that $V$ is an object in $\C$. If:
                $$R := \sum_{i_1 + i_2 = i} R_{i_1} \tensor R_{i_2} \in \End_{\C}(V^{\tensor 2})$$
            then we will write:
                $$R_{1, 2} := \sum_{i_1 + i_2 = i} R_{i_1} \tensor R_{i_2} \tensor \id_V \in \End_{\C}(V^{\tensor 3})$$
                $$R_{2, 3} := \sum_{i_1 + i_2 = i} \id_V \tensor R_{i_1} \tensor R_{i_2} \in \End_{\C}(V^{\tensor 3})$$
                $$\text{etc.}$$
        \end{convention}
        ...
        \begin{definition}[R-matrices] \label{def: R_matrices}
            An \textbf{R-matrix} on an object $V \in \Ob(\C)$ is an automorphism $R \in \Aut_{\C}(V \tensor V)$ satisfying the so-called \textbf{Yang-Baxter equation}:
                $$R_{1, 2} \circ R_{1, 3} \circ R_{2, 3} = R_{2, 3} \circ R_{1, 3} \circ R_{1, 2} \in \Aut_{\C}(V \tensor V \tensor V)$$
        \end{definition}
        \begin{example}
        ...
        \end{example}
        \begin{remark}
            Finding all solutions to the Yang-Baxter equation is a very difficult task. To see why, specialise to the case $\C := k\mod^{\fr}$ (the category of free $k$-modules/$k$-vector spaces) and write the equation out in terms of a fixed basis. 
            
            Nevertheless, it is known how one might generate new solutions from a previously known one (e.g. $R = \tau_{V, V}$). In particular, if $V$ is a $k$-vector space and $R \in \Aut_k(V \tensor V)$ is an R-matrix then the following automorphisms on $V \tensor V$ will also be R-matrices:
                $$R^{-1}, \lambda R, \tau_{V, V} \circ R \circ \tau_{V, V}$$
